...
def readfile(fn):
    with io.open(fn, 'r', encoding='utf8', newline=None) as f:
        return f.read()
...
def loaderMD(var):
    fn = var.group(1)
    # use different MD.dat's for python 2 vs 3 incase user switches versions,
    ...
    if fn in files and filemd5 in files[fn]:
        # use the stored copy if nothing has changed to avoid hitting the api more then the 60/hour
        # when not signed in
        db.close()
...
def loaderImage(var):
    ...
    return 'data:image/png;base64,{0}'.format(
        base64.encodebytes(open(fn, 'rb').read()).decode('utf8').replace('\n', '')
    )
...
def doReplacements(script, updateUrl, downloadUrl, pluginName=None):
    ...
    if resourceUrlBase:
        ...
        if '@@RESOURCEURLBASE@@' in script:
            raise Exception(
                "Error: '@@RESOURCEURLBASE@@' found in script, but no replacement defined"
            )
...
if buildMobile:
    ...
    try:
        ...
        pass
    shutil.copy(
        os.path.join(outDir, "total-conversion-build.user.js"),
        "mobile/assets/total-conversion-build.user.js"
    )
    # copy the user location script into the mobile folder.
    shutil.copy(
        os.path.join(outDir, "user-location.user.js"),
        "mobile/assets/user-location.user.js"
    )
    # also copy plugins
    try:
    ...
    if buildMobile != 'copyonly':
        ...
        if retcode != 0:
            ...
            shutil.copy(
                "mobile/bin/IITC_Mobile-%s.apk" % buildMobile,
